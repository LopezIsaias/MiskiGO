    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Link
          ...
          className="text-sm text-miski-olive hover:text-miski-forest flex items-center gap-1"
        >
          ← Volver
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-miski-forest">
            Pedido <span className="font-mono text-miski-olive">#{order.id.slice(0, 8).toUpperCase()}</span>
          </h1>
          <p className="text-xs text-miski-olive">{formatDateTime(order.created_at)}</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-xl border border-miski-sage/40 shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-1">
              ...
              {order.customer?.email && (
                <p className="text-sm text-miski-olive">{order.customer.email}</p>
              )}
            </div>
          </section>

          {/* Order items */}
          <section className="overflow-hidden rounded-xl border border-miski-sage/40 bg-white shadow-sm">
            <div className="px-4 py-3 border-b border-miski-sage/20 bg-miski-forest/5">
              ...
              <tbody className="divide-y divide-miski-sage/10">
                {order.order_items.map(item => (
                  <tr key={item.id} className="hover:bg-miski-cream/30">
                    ...
                    <td className="px-4 py-3 border-b border-miski-sage/20 text-right text-gray-700">
                      {item.quantity}
                    </td>
                    <td className="px-4 py-3 border-b border-miski-sage/20 text-right text-gray-700">
                      {formatCurrency(item.unit_price_frozen)}
                    </td>
                    <td className="px-4 py-3 border-b border-miski-sage/20 text-right font-bold text-miski-forest">
                      {formatCurrency(item.subtotal_frozen)}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t border-miski-sage/20 bg-miski-forest/5">
                {order.delivery_fee > 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-2 text-xs text-miski-olive text-right">Delivery</td>
                    <td className="px-4 py-2 text-right text-sm text-gray-700">{formatCurrency(order.delivery_fee)}</td>
                  </tr>
                )}
                {walletUsed > 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-2 text-xs text-miski-olive text-right">Billetera aplicada</td>
          ...
          <section className="bg-white rounded-xl border border-miski-sage/40 shadow-sm overflow-hidden">
            ...
            <div className="p-4 space-y-2 text-sm">
              <div>
                <span className="text-xs text-miski-olive block">Dirección</span>
                <span className="text-gray-700">{order.delivery_address}</span>
              </div>
              {deliveryDate && (
                <div>
                  <span className="text-xs text-miski-olive block">Fecha estimada</span>
                  <span className="text-gray-700">{deliveryDate}</span>
                </div>
              )}
              {order.delivery_notes && (
                <div>
                  <span className="text-xs text-miski-olive block">Notas de entrega</span>
                  <span className="text-gray-700">{order.delivery_notes}</span>
                </div>
              )}
              {order.customer_note && (
                <div>
                  <span className="text-xs text-miski-olive block">Nota del cliente</span>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="bg-miski-cream/30 rounded-lg p-3">
                  <p className="text-xs text-miski-olive mb-1">Método</p>
                  <p className="font-medium text-miski-forest">
                    {METHOD_LABEL[order.payment_method ?? ''] ?? order.payment_method ?? '—'}
                  </p>
                </div>
                <div className="bg-miski-cream/30 rounded-lg p-3">
                  <p className="text-xs text-miski-olive mb-1">Monto del comprobante</p>
                ...
                {pv && (
                  <div className="bg-miski-cream/30 rounded-lg p-3">
                    <p className="text-xs text-miski-olive mb-1">Enviado</p>
              ...
              {order.payment_method === 'wallet' || (!order.payment_proof_url && walletUsed >= order.total_amount) ? (
                ...
                <div>
                  {isProofImage ? (
                    <div className="relative w-full aspect-[3/4] rounded-lg overflow-hidden border border-miski-sage/40 bg-miski-cream/20">
                      <Image
                    ...
                    <a
                      ...
                      className="flex items-center gap-2 border border-miski-sage/40 rounded-lg px-4 py-3 text-sm text-miski-forest hover:bg-miski-cream/30 transition-colors"
                    >
                      <span>📄</span>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block text-xs text-miski-olive hover:text-miski-forest mt-1 text-center"
                  >
