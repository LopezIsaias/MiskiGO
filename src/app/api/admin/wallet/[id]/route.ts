...
export async function PATCH(
  ...
  const { data: tx } = await adminClient
    ...
    .select('id, user_id, amount, type, status, notes')
    .eq('id', txId)
    .neq('type', 'recharge')
    .eq('status', 'pending')
    .maybeSingle()
  ...
  if (parsed.data.action === 'approve') {
    ...
    await adminClient.from('audit_log').insert({
      ...
      action:       AUDIT_ACTIONS.WALLET_CREDIT_APPROVED,
      module:       AUDIT_MODULES.WALLET,
      entity_type:  'wallet_transaction',
      entity_id:    txId,
      ip_address:   ip,
      new_value:    { customer_id: tx.user_id, amount: tx.amount, type: tx.type, balance_before: balanceBefore, balance_after: balanceAfter },
    })
  } else {
    ...
    await adminClient.from('audit_log').insert({
      user_id:      user.id,
      role_at_time: profile!.role,
      action:       AUDIT_ACTIONS.WALLET_CREDIT_REJECTED,
      module:       AUDIT_MODULES.WALLET,
      entity_type:  'wallet_transaction',
      entity_id:    txId,
      ip_address:   ip,
      new_value:    { customer_id: tx.user_id, amount: tx.amount, type: tx.type, reason: parsed.data.reason },
    })
  }
