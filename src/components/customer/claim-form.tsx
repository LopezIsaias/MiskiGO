  ...
  if (submitted) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center space-y-3">
        ...
        <p className="text-xs text-green-700">El operador revisará tu caso y se comunicará contigo.</p>
        <a href="/customer/orders" className="inline-block mt-2 text-sm text-green-700 underline">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                <label className={`flex items-center gap-3 ${item.alreadyClaimed ? 'cursor-default' : 'cursor-pointer'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">{item.orderedQuantity} {item.unit}</p>
                  </div>
                  {item.alreadyClaimed && (
                  ...
                  <div className="mt-3 space-y-3 pt-3 border-t border-red-100">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full text-sm border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:border-red-400 resize-none"
                      />
                    </div>
                  </div>
                )}
      ...
      <button
        ...
        className="w-full py-3 rounded-xl text-sm font-semibold text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      >
        {submitting
          ? 'Enviando…'
          : selectedCount > 0
