  ...
  return (
    <div className="px-4 py-3 space-y-3">
      {suppliers.map(supplier => {
        ...
        return (
          <div key={supplier.supplierId} className="bg-white rounded-xl border border-miski-border shadow-sm overflow-hidden">
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {!supplier.isComplete && pendingCount > 0 && (
                  <span className="bg-miski-gold-light/40 text-miski-forest font-semibold text-xs px-2 py-0.5 rounded-full">
                    Pendiente
                  </span>
                )}
                <span className="text-miski-border text-sm">{isOpen ? '▲' : '▼'}</span>
              </div>
            </button>

            {/* Expanded form */}
            {isOpen && (
              <div className="border-t border-miski-border bg-miski-cream/10 px-4 pb-5 pt-4">
                {supplier.isComplete ? (
                  <div className="text-center py-4">
                    <p className="text-sm text-miski-forest font-semibold">Recepción registrada correctamente.</p>
                    <p className="text-xs text-miski-muted mt-1">Todos los productos de este proveedor han sido recibidos.</p>
                  </div>
                ) : (
