...
const MODULE_COLOR: Record<string, string> = {
  ...
  products:   'bg-miski-gold-light/40 text-amber-800',
  deliveries: 'bg-indigo-100 text-indigo-700',
  claims:     'bg-red-100 text-red-700',
  system:     'bg-miski-sage/30 text-miski-forest/70',
}
...
export function AuditTable({ rows }: Props) {
  ...
  return (
    <>
      {rows.length === 0 ? (
        <p className="text-sm text-miski-olive bg-white rounded-xl border border-miski-sage/40 p-8 text-center">
        ...
        <div className="overflow-hidden rounded-xl border border-miski-sage/40 bg-white">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-miski-sage/30 bg-miski-forest/5 text-xs text-miski-forest/60 font-semibold uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-miski-sage/20">
                {rows.map(row => (
                  <tr key={row.id} className="hover:bg-miski-cream/30 transition-colors">
                    <td className="px-4 py-3 text-xs text-miski-olive whitespace-nowrap border-b border-miski-sage/20">
                      {formatDateTime(row.timestamp)}
                    </td>
                    <td className="px-4 py-3 text-sm font-medium text-gray-800 whitespace-nowrap border-b border-miski-sage/20">
                      {row.actor?.full_name ?? '—'}
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-600 max-w-xs border-b border-miski-sage/20">
                      {describeAuditAction(row)}
                    </td>
                    <td className="px-4 py-3 text-xs text-miski-olive/70 font-mono whitespace-nowrap border-b border-miski-sage/20">
                      {row.action}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap border-b border-miski-sage/20">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${MODULE_COLOR[row.module] ?? 'bg-miski-sage/30 text-miski-forest/70'}`}>
                        {row.module}
                      </span>
                    </td>
      ...
      {selected && (
        ...
        >
          ...
          >
            {/* Modal header */}
            <div className="sticky top-0 bg-white px-5 py-4 border-b border-miski-sage/30 flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-miski-forest font-mono">{selected.action}</p>
                <p className="text-xs text-miski-olive mt-0.5">{formatDateTime(selected.timestamp)}</p>
              </div>
              <button
                type="button"
                onClick={() => setSelected(null)}
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                <div>
                  <dt className="text-miski-olive">Usuario</dt>
                  <dd className="font-medium text-gray-800">{selected.actor?.full_name ?? '—'}</dd>
                </div>
                <div>
                  <dt className="text-miski-olive">Rol</dt>
                  <dd className="font-medium text-gray-800">{selected.role_at_time}</dd>
                </div>
                <div>
                  <dt className="text-miski-olive">Módulo</dt>
                  <dd className="font-medium text-gray-800">{selected.module}</dd>
                </div>
                {selected.entity_type && (
                  <div>
                    <dt className="text-miski-olive">Entidad</dt>
                    <dd className="font-medium text-gray-800">
                      {selected.entity_type}
                      {selected.entity_id && (
                        <span className="ml-1 text-miski-olive font-mono">{selected.entity_id.slice(0, 8)}…</span>
                      )}
                    </dd>
                  </div>
                )}
                {selected.ip_address && (
                  <div>
                    <dt className="text-miski-olive">IP</dt>
                    <dd className="font-mono text-gray-800">{selected.ip_address}</dd>
                  </div>
                )}
                {selected.notes && (
                  <div className="col-span-2">
                    <dt className="text-miski-olive">Notas</dt>
              ...
              {selected.previous_value !== null && (
                <div>
                  ...
                  <pre className="bg-miski-sage/10 border border-miski-sage/30 rounded-lg p-3 text-xs overflow-x-auto text-gray-700 leading-relaxed">
              ...
              {selected.previous_value === null && selected.new_value === null && (
                <p className="text-xs text-miski-olive italic">Sin valores de cambio registrados.</p>
              )}
            </div>
          </div>
        </div>
