...
describe.skipIf(!INTEGRATION_ENABLED)('Numeración de comprobantes (RPC + constraints)', () => {
  ...
  it('receipts.number es único: insertar duplicado falla', async () => {
    ...
    const orderA = await createOrder(svc, { customerId: SUPERADMIN_ID, cycleId, regionId, status: 'delivered' })
    const orderB = await createOrder(svc, { customerId: SUPERADMIN_ID, cycleId, regionId, status: 'delivered' })
    const dupNumber = `U001-${Date.now().toString().slice(-8)}`

    const base = {
      customer_id: SUPERADMIN_ID, type: 'boleta', series: 'U001', correlative: 1,
  ...
  it('receipts.order_id es 1:1: dos comprobantes para un pedido fallan', async () => {
    ...
    const base = {
      order_id: orderId, customer_id: SUPERADMIN_ID, type: 'boleta', series: 'V001',
      document: '12345678', customer_name: 'Prueba', subtotal: 100, total: 100,
    }
