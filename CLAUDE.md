...
- **Sustitución (implementado 2026-06-24, migración 040):** por un ítem `failed` en pedido pagado, el operador puede PROPONER un producto alternativo (en vez de solo reembolsar). El cliente lo ACEPTA o RECHAZA desde la app (consentimiento → respeta inmutabilidad post-pago). Tope de precio: el sustituto nunca cuesta más que el original (`charged ≤ original`); si es menor, la diferencia va como crédito de billetera PENDIENTE que aprueba el superadmin (§3). Al aceptar: descuento atómico de stock, asignación `confirmed`, se repunta el `order_item` (producto/precio/subtotal congelados, status `assigned`), se cancela el reembolso Fase-3 del ítem y avanza el pedido; `orders.total_amount` NO cambia (= lo pagado). Al rechazar/expirar cae al reembolso Fase-3. Tabla `order_item_substitutions`. Si TODOS los ítems fallan y no se sustituyen, el pedido queda `failed` (Fase 3).
...
**Fecha:** 2026-06-24
**Pasos completados:** 1 al 20 — MVP completo + mejoras posteriores (ver CHANGELOG). Sesión 2026-06-22: **Fase 0 seguridad**, **Fase 1 demanda-primero** (catálogo desde `cycle_offerings`), **Fase 2** (captura on-behalf + auto-asignación confirmada), **Fase 3** (ítem failed en pedido pagado → aviso al cliente + propuesta de reembolso pendiente que aprueba el superadmin; pedido→failed si todos fallan). El approve-path usa `autoSourceOrderConfirmed`. **Sesión 2026-06-23: validación end-to-end en prod (sin cambios de código).** Happy path completo (ciclo→ofertas→pubs capturadas→pedido pagado→asignación cheapest-first/margen/congelado/stock→in_transit→delivered→completed) y path Fase-3 (gap parcial→ítem failed TODO-O-NADA con stock restaurado vía RPC→reembolso pending→pedido failed→aviso) verificados contra prod. RPCs `decrement/restore_publication_stock` OK. Datos de prueba eliminados; tablas operativas + `audit_log` truncadas (TRUNCATE: `wallet_transactions`/`audit_log` son inmutables a DELETE) → prod en baseline. Modelo: demanda-primero, catálogo fijo, se mantiene corte/ciclos; proveedor offline → operador siembra ofertas y captura sourcing.
**Validación integral 2026-06-23 (sesión b):** todos los caminos de negocio verificados — suite **72 unit + 50 integración en verde** (asignación, Fase-3, RPC stock concurrente, triggers lock/ventana/inmutabilidad, comprobantes, RLS) + spot-checks en prod replicando rutas vía SQL/RPCs reales (pago aprobar/rechazar, cancelación post-pago, aprobación de reembolso superadmin, recepción shortfall, reclamo wallet_credit, reserva anti-sobreventa, cron confirmed→failed). **Añadido endpoint** `POST /api/admin/orders/[id]/reopen-claim` (superadmin reabre ventana de reclamo; cerraba un gap: la constante/descr. existían sin ruta). Lint + tsc OK. Prod re-limpiada al baseline. **Siguiente fase acordada: parte gráfica (UI/UX).**
**Parte gráfica 2026-06-23 (sesión c) — identidad visual "Selva alta" en TODA la app:** sistema de marca (tokens `@theme`, fuentes Bricolage/Hanken/Geist Mono, componentes base en `src/components/ui/`, `/style-guide`) + aplicada a auth + los 5 roles (cliente, operador, proveedor, repartidor, admin). Paleta: forest/verde/lima/maíz/hueso/tinta/border; dinero en mono tabular; signature "sin escalas". Lógica intacta; lint+tsc verde. Commits `fc318a3`→`dee1135`. Pendiente UI: wiring de botón reopen-claim en panel claims; revisar responsive/mobile de los shells de escritorio; afinar badges de estado (siguen usando lima/gold de marca).
**Sesión 2026-06-24 (UI + sustitución):** del backlog del 24 — (1) **botón reopen-claim** cableado en panel de reclamos (superadmin); (2) **responsive mobile-first**: `SidebarShell` (drawer + hamburguesa) en los 4 sidebars de rol + layouts `md:flex`/`pt-20` (repartidor sin sidebar, sin cambios); (3) **badges de estado** unificados a escala semántica (ámbar/verde/rojo), lima/gold queda solo para marca. Commit `f745196`, push a prod (`0fb5dac..f745196`). Luego **Task 4 — Sustitución (§4)**: migración **040** `order_item_substitutions` aplicada en prod (MCP), backend `lib/utils/substitution.ts`, 3 rutas API, UI operador (`SubstitutionProposer`) + cliente (`SubstitutionProposalBanner`), 3 audit actions, test unit (suite **78 unit en verde**). Decisiones: cliente consiente; tope de precio al original (diff→crédito billetera pendiente, nunca cobra de más). Lint+tsc verde.
**Último commit:** `f745196` (UI backlog) — Task 4 pendiente de commit en esta sesión. `origin/main` — recordar `git push` para desplegar en Vercel.
**Migraciones aplicadas:** `030`–`040` TODAS en producción (`030`–`039` vía `db push`; **`040` vía MCP `apply_migration`**, tipos regenerados con `generate_typescript_types`). (Fases 2/3, cierre delivered→completed y tooling son código, sin migración.)
**Limpieza de prod:** HECHA 2026-06-22 vía MCP Supabase. Backup completo previo en schema **`backup_20260622`** (24 tablas, copia 1:1 verificada conteo-a-conteo — conservar unos días; restaurar con `INSERT INTO public.X SELECT * FROM backup_20260622.X`; borrar con `DROP SCHEMA backup_20260622 CASCADE`). Truncado todo lo operativo + `audit_log`; conservados users(13)/products(10)/categorías(13)/regiones(1)/system_params(2); saldos→0, reputación→100. Prod limpia y verificada.
**Próximo paso (operativo, panel — aquí continuamos):** (1) Abrir un ciclo + sembrar ofertas en *Ofertas del ciclo* → catálogo se llena. (2) *Captura de oferta* → registrar oferta de proveedores + "Asignar pedidos del ciclo". (3) Probar el flujo end-to-end en prod (pedido cliente → asignación → entrega → cierre). Para pruebas locales: `npm run seed:example` (demo, solo local). Futuro: **sustitución** (producto alternativo en vez de solo reembolsar). Menor: "entregado a la primera" solo se distingue por `delivery_attempts`, no por el status final.
...
**Decisiones pendientes:** ~~Edge case — proveedor rechaza TODOS los ítems sin reemplazo → pedido atascado en `confirmed`~~ → RESUELTO 2026-06-22 (Fase 3). ~~Sustitución (producto alternativo) = futuro~~ → IMPLEMENTADO 2026-06-24 (migración 040; ver §4). Nota: §4 — el cliente puede SOLICITAR cancelación post-pago en estado `confirmed`; operador/superadmin ejecuta; reembolso MANUAL. **Sustitución — pendientes menores:** no hay auto-expiración de propuestas viejas (sin cron); la propuesta descuenta stock recién al ACEPTAR (si se agota entre propuesta y aceptación, la aceptación falla limpiamente). Tests de integración de sustitución no corridos (requieren stack Supabase local); ruta de precio cubierta por unit test.
