...
**Último commit:** `dee1135` (identidad proveedor/repartidor/admin). `origin/main` — recordar `git push` para desplegar en Vercel. 
**Migraciones aplicadas:** `030`–`039` TODAS en producción vía `db push`. (Fases 2/3, el cierre delivered→completed y el tooling son código, sin migración.)
...
**Decisiones pendientes:** ~~Edge case — proveedor rechaza TODOS los ítems sin reemplazo → pedido atascado en `confirmed`~~ → RESUELTO 2026-06-22 (Fase 3): ítem sin stock en pedido pagado → propuesta de reembolso pendiente + aviso; si todos fallan, pedido→`failed`. Sustitución (producto alternativo) = futuro. Nota: §4 — el cliente puede SOLICITAR cancelación post-pago en estado `confirmed`; operador/superadmin ejecuta; reembolso MANUAL.
